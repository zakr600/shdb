...
void validateBTree(shdb::BTree & btree, const std::vector<std::pair<shdb::Row, shdb::RowId>> & inserted_entries)
{
    ...
    if (entries.size() != inserted_entries_copy.size()) {
        std::cout << "sizes\n";
        ASSERT_TRUE(false);
    }
    for (int i = 0; i < entries.size(); ++i) {
        if (entries[i] != inserted_entries_copy[i]) {
            std::cout << entries[i].first << " " << entries[i].second.row_index << std::endl;
        }
    }
...
TEST(BTree, BTreeInsertLookupSimpleKey)
{
...
TEST(BTree, BTreeInsertLookupComplexKey)
{
    static size_t iterations = 5;
